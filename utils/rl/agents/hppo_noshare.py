...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        # 离散
        discrete_prob = F.softmax(self.l3_1(x_1), dim=0)
        # 连续
...
class PPO(object):
    ...
    def train(self, replay_buffer, batch_size=32, c_epoch=10, a_epoch=10):
        ...
        losses = {
            'c_loss': sum(c_loss_list) / c_epoch,
            'a_loss': sum(a_loss_list) / a_epoch,
            'discrete_a_loss': sum(discrete_a_loss_list) / a_epoch,
            'parameter_a_loss': sum(parameter_a_loss_list) / a_epoch,
        }
